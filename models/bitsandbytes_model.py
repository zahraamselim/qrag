"""
BitsAndBytes Model Implementation

Implements ModelInterface for NF4/INT8 quantized models using BitsAndBytes.
Focus: On-the-fly quantization with optimal normal float representation.
"""
...
class BitsAndBytesModel(ModelInterface):
    """BitsAndBytes NF4/INT8 model implementation"""
    ...
    def load(self):
        ...
        print(f"Loading model with BitsAndBytes: {self.model_path}")
        ...
        if self.load_in_4bit:
            ...
            print("Quantization: INT8 8-bit")
            bnb_config = BitsAndBytesConfig(
                load_in_8bit=True
            )
        else:
            raise ValueError("Must specify either load_in_4bit or load_in_8bit")
        ...
        quant_type = "NF4 4-bit" if self.load_in_4bit else "INT8 8-bit"
        print(f"Model loaded: {quant_type}")
        print(f"Device: {self._model.device}")
        print(f"Type: {self._model_type}")
    
    def generate(self, prompt: str, config: GenerationConfig, return_attentions: bool = False) -> ModelOutput:
        """Generate text from prompt."""
        if return_attentions:
            print("Warning: Attention extraction with BitsAndBytes adds overhead")
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        quant_method = "nf4_4bit" if self.load_in_4bit else "int8_8bit"
